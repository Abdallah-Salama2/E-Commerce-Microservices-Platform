...
export const getCacheVersion = async (versionKey, defaultVersion = "1") => {
  try {
    ...
    // لو الـ Version مش موجود، نكريته
    await redis.set(versionKey, defaultVersion);
    return defaultVersion;
    
  } catch (err) {
    
    getLogger().warn({ err, key: versionKey }, "Failed to get/set cache version, using default");
    return defaultVersion;
  }
};