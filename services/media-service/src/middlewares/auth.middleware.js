...
export const optionalAuthenticate = (req, res, next) => {
  try {
    const authHeader = req.headers?.authorization;
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return next(); // guest — proceed with req.user undefined
    }
    const accessToken = authHeader.split(" ")[1];
    req.user = jwt.verify(accessToken, env.JWT.ACCESS_SECRET);
    next();
  } catch (err) {
    // bad/expired token on a public route shouldn't 401 — just treat as guest
    req.user = null;
    next();
  }
};
