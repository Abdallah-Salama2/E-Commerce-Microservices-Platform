...
// Keyed by orderId — every event this service publishes describes
// something about one order, so this is the correct partition key.
export const toMessage = (row) => {
  const parsedRow = JSON.parse(row.payload);
  return {
    key: String(parsedRow.orderId),
    value: row.payload,
  };
};
